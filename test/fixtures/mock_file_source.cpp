...
std::unique_ptr<FileRequest> MockFileSource::request(const Resource& resource, Callback callback) {
    auto req = std::make_unique<MockFileRequest>(resource, *this);
    req->workRequest = thread_->invokeWithCallback(&Impl::handleRequest, callback, req.get(), resource);
    return std::move(req);
}

void MockFileSource::cancel(FileRequest* request) {
    thread_->invoke(&Impl::cancelRequest, request);
}
