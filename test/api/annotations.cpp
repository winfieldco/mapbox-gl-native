...
std::shared_ptr<SpriteImage> defaultMarker() {
    util::Image image(util::read_file("test/fixtures/sprites/default_marker.png"));
    return std::make_shared<SpriteImage>(image.getWidth(), image.getHeight(), 1.0, std::string(image.getData(), image.getWidth() * image.getHeight() * 4));
}
